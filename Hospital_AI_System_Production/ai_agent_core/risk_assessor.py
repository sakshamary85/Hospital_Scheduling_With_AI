...
class RiskAssessor:
    ...
    def assess_risk(self, ml_prediction: str, show_probability: float, no_show_probability: float) -> RiskLevel:
        """
        Assess patient risk level based on ML prediction and probabilities.
        
        Args:
            ml_prediction: ML model prediction ("Show" or "No-show")
            show_probability: Probability of patient showing up (0.0 to 1.0)
            no_show_probability: Probability of patient not showing up (0.0 to 1.0)
            
        Returns:
            RiskLevel enum value
        """
        # Primary logic: Trust ML model prediction
        if ml_prediction == "Show" and show_probability >= 0.6:
            # ML predicts SHOW with high confidence (≥60%)
            return RiskLevel.LOW
        elif ml_prediction == "No-show" or show_probability < 0.4:
            # ML predicts NO-SHOW or very low show confidence (<40%)
            return RiskLevel.HIGH
        else:
            # Uncertain prediction (40-60% show probability)
    ...
    def _low_risk_strategy(self, slot_available: bool, slot_capacity: int) -> Dict[str, Any]:
        ...
        if slot_available and slot_capacity > 0:
            ...
            return {
                ...
                "waitlist_priority": 1,
                "interventions": ["standard_reminder"],
                "notes": "Low risk patient - add to standard waitlist"
            }
    ...
    def calculate_waitlist_priority(self, 
        ...
        risk_level = self.assess_risk(ml_prediction, show_probability, no_show_probability)
    ...
    def get_risk_summary(self, ml_prediction: str, show_probability: float, no_show_probability: float) -> Dict[str, Any]:
        """
